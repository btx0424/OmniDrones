...
def compute_control(
    ...
    target_y_ax = torch.cross(target_z_ax, target_x_c, dim=-1) / torch.norm(torch.cross(target_z_ax, target_x_c, dim=-1), dim=-1, keepdim=True)
