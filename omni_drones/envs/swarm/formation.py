...
#             "others": UnboundedContinuousTensorSpec((self.drone.n-1,  3 + 3 + 1)).to(self.device)
#         })
...
#         self.init_pos_scale = torch.tensor([0.2, 0.2, 0.2], device=self.device)
#         self.init_pos_trans = torch.tensor([-.1, -.1, -0.1], device=self.device)
...
#         pos = pos[env_ids] 
#         offset = torch.randn_like(pos) * self.init_pos_scale + self.init_pos_trans
#         vel = torch.zeros(len(env_ids), self.drone.n, 6, device=self.device)
#         self.drone.set_world_poses(pos + offset, rot[env_ids], env_ids)
...
#         )
#         com_pos = (pos - self.envs_positions[env_ids].unsqueeze(1)).mean(-2, keepdim=True)
#         self.last_cost_pos[env_ids] = torch.square(
#             com_pos - self.target_pos
#         ).sum(-1)
...
#         return TensorDict({"drone.obs": obs, "drone.state": state}, self.batch_size)
...
#         reward[:] = (
#             50 * (self.last_cost_l - cost_l)
#             + 50 * (self.last_cost_h - cost_h)
#             + (self.last_cost_pos - cost_pos)
#             - self.drone.base_link.get_net_contact_forces().any().float()
#         ).unsqueeze(-1)
...
class Formation(IsaacEnv):
    def __init__(self, cfg, headless):
        ...
        self.init_pos_scale = torch.tensor([0.4, 0.4, 0.2], device=self.device)
        self.init_pos_trans = torch.tensor([-.2, -.2, -0.1], device=self.device)
