...
class ParallelAct(nn.Module):
    def __init__(self, 
        ...
        self.act_dist_module = act_dist_module
    
    def forward(self):
        ...

class AutoregressiveAct(nn.Module):
...
class MATPolicy(object):
    def __init__(self) -> None:
        ...
        self.obs_name = f"obs"
        self.value_name = f"state_value"
        self.act_name = f"action"
        
        TensorDictModule(
            Encoder(), 
            [self.obs_name], 
            [self.value_name, "encoder_feature"]
        )
        TensorDictModule(
            Decoder(), 
            ["encoder_feature", self.act_name], 
            ["decoder_feature"]
        )
