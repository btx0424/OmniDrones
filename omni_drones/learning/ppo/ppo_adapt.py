...
class PPOAdaptivePolicy:
    ...
    def __init__(self, cfg: PPOConfig, agent_spec, device):
        ...
        intrinsics_dim = agent_spec.state_spec.shape[-1]

        fake_input = CompositeSpec({
            "agents": {
                "observation": agent_spec.observation_spec,
                "intrinsics": agent_spec.state_spec
            }
        }).zero()

        actor_module = TensorDictSequential(
            TensorDictModule(
                make_mlp([128, 128]), [("agents", "observation")], ["obs"]
            ),
            TensorDictModule(
                nn.Sequential(nn.LayerNorm(intrinsics_dim), make_mlp([64, 64])), 
                [("agents", "intrinsics")], ["context"]
            ),
            CatTensors(["obs", "context"], "feature"),
            TensorDictModule(
                Actor(self.action_dim), ["feature"], ["loc", "scale"]
            )
        )
    ...
    def __call__(self, tensordict: TensorDict):
        ...
        tensordict.exclude("feature", "loc", "scale", inplace=True)
        return tensordict
