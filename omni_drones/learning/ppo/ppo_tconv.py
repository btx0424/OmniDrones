...
class TConv(nn.Module):
    def __init__(self) -> None:
        super().__init__()
        self.projection = make_mlp([64, 64])
        self.tconv = nn.Sequential(
            nn.Conv1d(64, 64, kernel_size=7, stride=2), nn.ELU(),
    ...
    def forward(self, features: torch.Tensor):
        ...
        features = self.projection(features.transpose(1, 2)).transpose(1, 2)
...
class PPOTConvPolicy:
    ...
    def __init__(
        ...
        actor = TensorDictModule(
            nn.Sequential(TConv(), Actor(self.action_dim)), 
            [("agents", "observation_h")], ["loc", "scale"]
        )
        ...
        self.critic = TensorDictModule(
            nn.Sequential(TConv(), nn.LazyLinear(1)), 
            [("agents", "observation_h")], ["state_value"]
        ).to(self.device)
