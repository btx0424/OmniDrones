...
class TanhIndependentNormal(D.TransformedDistribution):
    ...
    def log_prob(self, value: torch.Tensor):
        return super().log_prob(((value - self._loc) / self._scale).clamp(-1 + self.eps, 1 - self.eps))
...
class IndependentBeta(D.Independent):
    def __init__(self,
        alpha: torch.Tensor,
        beta: torch.Tensor,
        min: Union[float, torch.Tensor] = 0.,
        max: Union[float, torch.Tensor] = 1.,
        event_dims: int = 1,
    ):
    ...
    def log_prob(self, value: torch.Tensor):
        return super().log_prob(((value - self.min) / self.scale).clamp(self.eps, 1.-self.eps))
...
class IndependentNormalModule(nn.Module):
    ...
    def forward(self, tensor: torch.Tensor) -> Tuple[torch.Tensor]:
        if self.state_dependent_std:
            ...
            loc, scale = self.operator(tensor), self.scale_mapping(self.log_std).clamp_min(self.scale_lb)
...
class TanhIndependentNormalModule(nn.Module):
    def __init__(self,
        ...
        if isinstance(scale_mapping, str):
            ...
            raise ValueError("scale_mapping must be a string or a callable function.")
        self.scale_lb = scale_lb
        self.dist_cls = functools.partial(TanhIndependentNormal, min=min, max=max, event_dims=event_dims)
...
class IndependentBetaModule(nn.Module):
    def __init__(self,
        ...
        if isinstance(scale_mapping, str):
            ...
            raise ValueError("scale_mapping must be a string or a callable function.")
        self.dist_cls = functools.partial(IndependentBeta, min=min, max=max, event_dims=event_dims)
...
class MultiCategorical(D.Distribution):
    ...
    def sample(self, sample_shape=torch.Size()):
        ...
        return torch.stack([dist.sample(sample_shape) for dist in self.base_dists], dim=-1)
    ...
    def entropy(self):
        return torch.stack([dist.entropy() for dist in self.base_dists], dim=-1).sum(dim=-1)
...
class MultiCategoricalModule(nn.Module):
    def __init__(self,
        ...
        self.output_dims = output_dims.tolist() if isinstance(output_dims, torch.Tensor) else output_dims
