...
class ValueNorm1(Normalizer):
    def __init__(self, 
        ...
        self.input_shape = torch.Size(input_shape) if isinstance(input_shape, Iterable) else torch.Size((input_shape,))
        ...
        self.register_buffer("debiasing_term", torch.tensor(0.))
...
class ValueNorm2(Normalizer):
    ...
    def normalize(self, input_vector: torch.Tensor):
        ...
        if self.subtract_mean:
            return (input_vector - self.running_mean) / torch.sqrt(self.running_var + self.eps)
        else:
