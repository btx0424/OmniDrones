...
def split(input,history,root):
    train_input=input[:,:history*root]
    train_info=input[:,history*root:]
    ...
    for j in range(1):
        ...
        for k in range(200):
            ...
            for i,data in enumerate(train_data):
                ...
                train_obs,train_info=split(data_fedin,history_length,root_and_action)
                train_input=train_obs.reshape(-1,history_length,root_and_action)
                x=adapt(train_input)
                with torch.no_grad():
                    y=train_info
                loss=loss_func(x,y)
                ...
                optimizer.step()
                if m==100000:
                    torch.save(adapt.state_dict(),"outputs/adaptive_module.pt")
                    break
                if m % 1000 == 0:
                    with torch.no_grad(): 
                        ...
                        for l,test in enumerate(test_data):
                            ...
                            test_input=test_obs.reshape(-1,history_length,root_and_action)
                            y_test=test_info
